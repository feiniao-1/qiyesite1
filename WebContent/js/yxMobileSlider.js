...
(function($){
    $.fn.yxMobileSlider = function(settings){
        ...
        return this.each(function(){
            ...
            var oPosition = {}; //触点位置
            var w=$(".sliderout").width();
            var moveWidth = w; //滚动宽度
            //初始化主体样式
            _this.width(w).height(s.height).css({
                position: 'relative',
                overflow: 'hidden',
				margin:'0 auto'
            }); //设定容器宽高及样式
            ...
            $(window).bind('resize load', function(){
                ...
                oLi.width(w).height(_this.height());//设定滚动单元宽高
                oMover.width(num * oLi.width());
                oFocusContainer.width(w).height(_this.height() * 0.15).css({
                    zIndex: 2
                });//设定焦点容器宽高样式
                _this.fadeIn(300);
            });
            ...
            function mobileSettings(){
                moveWidth = w;
                var iScale = w / s.width;
                _this.height(s.height * iScale).width(w);
                oMover.css({
